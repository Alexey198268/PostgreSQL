...
(select substring ((select cast(last_update as text)) from 0 for 11)) as "Дата посл. обн. записи о пок-ле"
from customer c 
...
select upper(first_name), upper(last_name) from customer c 
...
select *
from film f 
where (rating = 'R' and rental_rate >=0 and rental_rate <= 3.00) or (rating = 'PG-13' and rental_rate >=4.00)
...
select *
from film f 
order by (select character_length(title)) desc
limit 3
...
select (select substring(email from 0 for (strpos(email, '@')))), 
(select substring(email from (strpos(email, '@'))+1 for (SELECT CHARACTER_LENGTH(email)))) 
from customer c 
